...
function buildPlainReport({ target, snapshot, hits, kills, gap, rewrites, rewriteLab = [], topThirdPacket = [], actionPlan = [] }) {
  ...
  lines.push(
    ...
    '',
    'Value receipt',
    `- You got a ${snapshot.score}/100 fit snapshot with ${snapshot.priorityFixes.length} priority fix${snapshot.priorityFixes.length === 1 ? '' : 'es'}.`,
    gap ? `- You got a job-post keyword gap with ${gap.missing.length} missing/buried term${gap.missing.length === 1 ? '' : 's'} to handle honestly.` : '- Paste a job post next time to add a local keyword-gap receipt.',
    '- You got a copyable 5-minute apply plan plus a recruiter-note starter.',
    '- Tip only if this saved at least 20 minutes or stopped one low-quality application.',
    '',
    `If this helped, tip $5 via Ko-fi: ${kofiUrl}`,
    'Disclosure: Built by Quark Assistant — autonomous AI agent. Code authored by AI under owner supervision. Feedback is generated by an AI-authored rules engine and is not legal, career, or hiring advice.',
...
function buildValueReceipt(snapshot, gap) {
  const keywordLine = gap
    ? `Job-post keyword gap checked: ${gap.present.length} matched, ${gap.missing.length} missing or buried.`
    : 'No job post pasted yet; rerun with the posting to get a local keyword-gap receipt.';
  return `
    <section class="value-receipt" aria-label="Value receipt before tipping">
      <h3>Value receipt before you tip</h3>
      <ul>
        <li>${snapshot.score}/100 fit snapshot with ${snapshot.priorityFixes.length} priority fix${snapshot.priorityFixes.length === 1 ? '' : 'es'}.</li>
        <li>${esc(keywordLine)}</li>
        <li>Copyable 5-minute apply plan plus a recruiter-note starter.</li>
      </ul>
      <p>Tip only if this saved at least 20 minutes or stopped one low-quality application.</p>
    </section>
  `;
}
...
async function shareProduct() {
  ...
  const payload = {
    ...
    text: 'Free browser-only resume fit snapshot and rewrite checklist: no upload, no storage.',
    url: productUrl,
  };
  try {
    if (navigator.share) {
    ...
    status.textContent = 'Share blurb copied. Send it to one relevant job seeker, not a spam blast.';
  } catch (err) {
...
async function copyShareBlurb() {
  ...
  try {
    await navigator.clipboard.writeText(shareBlurb);
    status.textContent = 'Share blurb copied. Send it where it is genuinely useful.';
  } catch (err) {
    status.textContent = `Copy this link: ${productUrl}`;
  }
}
...
function critique() {
  ...
  out.innerHTML = `
    ...
    ${buildValueReceipt(snapshot, gap)}
    ${tipCta}
  `;
...
document.getElementById('copy-share-blurb').addEventListener('click', copyShareBlurb);
